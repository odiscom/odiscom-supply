...
export default function Account() {
  ...
  async function loadCustomerData(customerEmail = email) {
    ...
    if (quoteError || orderError) { setMessage('Account activity could not be loaded. Totals are unavailable.'); setQuotes([]); setOrders([]); setLoading(false); return }
    setLoaded(true)
    setQuotes(quoteData || [])
    setOrders(orderData || [])
    setLoading(false)
      ...
      <main className="min-h-screen bg-slate-50">
        ...
        <section className="mx-auto max-w-6xl px-6 py-10">
          <div className="mb-6 grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="text-sm text-slate-500">Quotes</div><div className="mt-1 text-3xl font-bold text-slate-950">{loaded ? quotes.length : '—'}</div></div>
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="text-sm text-slate-500">Orders</div><div className="mt-1 text-3xl font-bold text-slate-950">{loaded ? orders.length : '—'}</div></div>
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="text-sm text-slate-500">Order Value</div><div className="mt-1 text-3xl font-bold text-slate-950">{loaded ? money(recordTotal(orders, 'total')) : '—'}</div></div>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="divide-y divide-slate-200">
                {loaded && quotes.length === 0 && <div className="p-8 text-slate-500">No quotes found.</div>}
                {quotes.map((quote) => (
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="divide-y divide-slate-200">
                {loaded && orders.length === 0 && <div className="p-8 text-slate-500">No orders found.</div>}
                {orders.map((order) => (
