      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
          ...
          <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            {loading ? <div className="p-10 text-slate-600">Loading suppliers...</div> : <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-slate-600"><tr><th className="px-5 py-4 text-left">Supplier</th><th className="px-5 py-4 text-left">Contact</th><th className="px-5 py-4 text-left">Categories</th><th className="px-5 py-4 text-left">Terms</th><th className="px-5 py-4 text-left">Lead Time</th><th className="px-5 py-4 text-right">Action</th></tr></thead><tbody>{filteredSuppliers.length === 0 && <tr><td colSpan="6" className="p-10 text-center text-slate-500">No suppliers found.</td></tr>}{filteredSuppliers.map((supplier) => <tr key={supplier.id} className="border-t align-top hover:bg-slate-50"><td className="px-5 py-4"><div className="font-semibold text-slate-950">{supplier.name}</div>{supplier.website && <a href={supplier.website} target="_blank" className="mt-1 block text-xs text-blue-700">{supplier.website}</a>}<div className="mt-3 max-w-sm whitespace-pre-wrap text-xs text-slate-500">{supplier.notes || ''}</div></td><td className="px-5 py-4"><div>{supplier.contact_name || '-'}</div><div className="mt-1 text-xs text-slate-500">{supplier.email || ''}</div><div className="text-xs text-slate-500">{supplier.phone || ''}</div></td><td className="px-5 py-4"><div className="flex max-w-xs flex-wrap gap-2">{String(supplier.product_categories || '-').split(',').map((cat) => cat.trim()).filter(Boolean).map((cat) => <Badge key={cat} tone="slate">{cat}</Badge>)}</div></td><td className="px-5 py-4">{supplier.payment_terms || '-'}</td><td className="px-5 py-4">{supplier.lead_time || '-'}</td><td className="px-5 py-4 text-right"><button type="button" onClick={() => deleteSupplier(supplier)} className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">Delete</button></td></tr>)}</tbody></table></div>}
          </div>
        </div>
      </div>
    </AdminShell>
